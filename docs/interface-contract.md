...
- Creates a Moodle Lesson content, truefalse, or multichoice question page through `lesson_page::create(...)` after validating that the module belongs to the selected course.
...
- `page_type` defaults to `content`. For `truefalse`, `answers` must contain exactly two entries, either as `correct` and `wrong` objects or an `answers` array. For `multichoice`, `answers` is a JSON object with optional `multi_answer` and an `answers` array containing at least two choices. Each answer supports `answer`, optional `answer_format`, optional `response`, optional `response_format`, optional `score`, and `jump_to` as an integer page id or one of the supported jump names.
...
- Context: Lesson module.
- Capabilities: `mod/lesson:manage`.
- Updates a Moodle Lesson content, truefalse, or multichoice question page through the Lesson page component `update(...)` method after verifying that the page belongs to the selected Lesson module.
